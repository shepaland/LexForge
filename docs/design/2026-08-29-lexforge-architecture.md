...
Этапы 1–3 велись через установленный `openspec` 1.9.0 в каталоге `openspec/`. Когда своё ядро
заработало, репозиторий перешёл на `lexforge init` и каталог `lexforge/`: переход проверил
продукт на реальном проекте. Каталога `openspec/` больше нет, все этапы, включая уже
пройденные, теперь идут через собственный конвейер.
