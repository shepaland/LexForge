...
const WITH_FINDING = [
  ...
  "Depends on: none",
  "",
  "- [ ] 1.1 [A] Написать хранение пароля в виде хеша и оставить TODO на соль",
  "      -> auth#Password is stored hashed",
  "",
].join("\n");
...
/**
 * Writes an already-split index (`tasks.md` plus its section files) and any
 * extra file overrides, with no further splitting of its own — the escape
 * hatch the CRLF tests below use, since they need the split done on plain
 * `\n` content and only convert to `\r\n` afterward, one file at a time.
 */
function workspaceFromIndex(
  index: string,
 ...
 * `tasks.md` keeps the heading and a link, and the file that link points at
 * keeps the section's own `Depends on:` line and its task, split out of
 * `tasks` (still written the old, flat way by `CLEAN` and `WITH_FINDING`)
 * by `splitPlanIntoIndex`.
 */
function workspace(tasks: string, files: Record<string, string> = {}): string {
  const { index, sections } = splitPlanIntoIndex(tasks);
  return workspaceFromIndex(index, sections, files);
}
...
describe("lexforge check plan на файлах с переводами строк Windows", () => {
  ...
  it("находит тот же плейсхолдер на той же строке, что и на файлах с переводами Unix", async () => {
    // Split on the plain `\n` fixture first, so the heading regex the
    // splitter runs still matches, then convert every piece to `\r\n` —
    // converting `WITH_FINDING` itself first would leave a trailing `\r` on
    // each line that the splitter's heading regex cannot match.
    const { index, sections } = splitPlanIntoIndex(WITH_FINDING);
    const windows = workspaceFromIndex(
      crlf(index),
      Object.fromEntries(
        Object.entries(sections).map(([target, content]) => [target, crlf(content)]),
      ),
  ...
  it("на чистом плане с переводами Windows отвечает кодом 0", async () => {
    const { index, sections } = splitPlanIntoIndex(CLEAN);
    const windows = workspaceFromIndex(
      crlf(index),
      Object.fromEntries(
        Object.entries(sections).map(([target, content]) => [target, crlf(content)]),
