 ...
 * файла в `lexforge-gate-commands` — для четырёх команд-ворот — и решениями 1 и 6
 * файла `design.md` в `models-per-runtime`: имя рантайма приходит флагом `--tool`,
 * поля роли в ответах больше нет. Если снимок разошёлся с решением, правится код,
 * а не снимок.
 */
